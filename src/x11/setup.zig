...
pub fn do(platform: anytype, display_info: DisplayInfo, auth_cookie: ?AuthCookie) !void {
    ...
    // Why not just use the platform recv/send buffers?
    // 1: the handshake message is quite a bit larger than all the others we will receive, so having a bigger buffer on the stack here makes sense
    // 2: if the handshake fails we won't waste syscalls mmap/munmap-ing the platform buffers
    var file = std.fs.File{ .handle = platform.socket };
    var rbuf = std.io.BufferedReader(4096 * 4, @TypeOf(std.fs.File.reader(undefined))){ .unbuffered_reader = file.reader() }; // About the size of my X11 handshake (AMD, 1 screen)
    var wbuf = std.io.BufferedWriter(4096, @TypeOf(std.fs.File.writer(undefined))){ .unbuffered_writer = file.writer() };
    ...
    if (display_info.screen >= server_handshake.roots_len) {
        std.log.scoped(.zigwin).err("X11 screen {} does not exist, max is {}", .{ display_info.screen, server_handshake.roots_len - 1 });
    ...
    if (@TypeOf(platform.*).settings.monitors == true) {
    ...
    try wbuf.flush();
    try handleEvents(platform, &rbuf, reader, &wbuf, writer);

    if (platform.ext_op_present == 0) return error.PresentExtensionNotSupported;
    if (platform.ext_op_mitshm == 0) return error.MitSHMExtensionNotSupported;
}
...
fn readServerHandshake(reader: anytype) !ServerHandshake {
    ...
    switch (response_header.status) {
        0 => {
            ...
            std.log.scoped(.zigwin).err("X11 handshake failed: {}", .{reason});
...
fn handleEvents(platform: anytype, rbuf: anytype, reader: anytype, wbuf: anytype, writer: anytype) !void {
